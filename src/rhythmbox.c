...
static InfParameters params = {.get_width = get_width,
    .set_width = set_width,
...
static void notify_critical_error(const gchar *message) {
    g_message("notify_critical_error TODO");
}
static void disable_plugin() {
    g_message("disable_plugin TODO");
}

static Player player = {.notify_critical_error = notify_critical_error, .disable_plugin = disable_plugin};
...
EXPORT void peas_register_types(PeasObjectModule *module) {
    rb_infinity_plugin_register_type(G_TYPE_MODULE(module));
    peas_object_module_register_extension_type(module, PEAS_TYPE_ACTIVATABLE, RB_TYPE_INFINITY_PLUGIN);
}
